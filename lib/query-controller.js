...
class QueryController {
    ...
    _advance(text, done) {
        ...
        debug("finding query by '%s'", this.session.query);
        this._index = this._queries.findIndex((query) => {
            return query.name === this.session.query;
        });
        ...
        return async.series([
            ...
            function(next) {
                ...
                const choices = _this.session.choices;
                if (choices) {
                    const found = choices.indexOf(text) !== -1;
                    if (!found) {
                        const retryText = question ? question.retryText : null;
                        return next(retryText || new Error(`Invalid choice (${text}). Please try again.`));
                    }
                }
                _this.session.answers[currentQuery.name] = text;
            ...
            const query = _this._retry ? currentQuery : nextQuery;
            if (!query || _this._stop) {
                return done();
            }
            _this.session.query = null;
            _this.session.choices = null;
            const question = Object.assign({}, query.question);
            if (_this._retryText) {
            ...
            if (question.choices) {
                ...
                if (!raw.length) {
                    return done(new Error("einvalid: No choices available"));
                }
                ...
                if (question.strict) {
                    _this.session.choices = choices.map(c => c.id);
                }
            }
            _this.session.query = query.name;
            return done(null, question, query);
        });
    }
    ...
    getAnswer(name) {
        ...
        return this.session.answers[name];
    }
