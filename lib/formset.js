...
class FormSet extends EventEmitter {
     ...
     */
    addForm(name, test, queries, cb, options={}) {
        if (test instanceof RegExp) {
            test = test.test.bind(test);
        } else if (test === null) {
            test = function() { return false; };
        }
        const form = new Form(name, test, queries, cb, options);
    ...
    _process(chatID, message, opts, done) {
        ...
        function onProcess(error, s, q) {
            ...
            session = s;
            query = q;
            // NOTE/impl: If 'query' is falsey, form is complete
            if (!query) {
                debug("destroying session: %s", sid);
                // TODO: what if session could not be destroyed?
                return _this.store.del(sid, onSessionDestroy);
            }
        ...
        function onSessionDestroy(error) {
            if (error) {
                debug("error deleting session");
                return done(error);
            }
            if (form.cb) {
                debug("firing form callback");
                form.cb(session.answers, ref);
            }
            return done(null);
        }
        function onSessionSave(error) {
            ...
            if (query) {
                ...
                // NOTE/i18n: internalize the text
                if (form.options.i18n) {
                    if (payload.text) {
                        payload.text = form.options.i18n(payload.text, null, message);
                    }
                    /*
                    if (payload.choices) {
                        payload.choices = payload.choices.map((choice) => {
                            return form.options.i18n(choice, null, ref);
                        });
                    }
                    */
                }
                _this.emit("query", payload, ref);
            }
