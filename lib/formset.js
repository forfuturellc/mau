...
class FormSet extends EventEmitter {
     ...
     * @param  {Number} [options.ttl=null] Time-To-Live for sessions
     */
    constructor(options={}) {
        super();
     ...
     */
    processForm(formName, chatID, ref, done) {
        const form = this._forms.find((form) => {
            return form.name === formName;
        });
        if (!form) {
            debug("form not found: %s", form.name);
            return done(new Error(`enoent: ${form.name}`));
        }
        return this._process(chatID, ref, { form }, done);
    }

    /**
     * Process a message.
     * 'error.code' equals 'ENOENT' if form was not found.
     * @param  {String|Number} chatID Unique identifier for the originating chat
     * @param  {String} text Text of the message
     * @param  {Object|Function} ref Reference
     * @param  {Function} done(error) Invoked on error
     */
